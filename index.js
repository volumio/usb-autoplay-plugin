...
ControllerUsbAutoplay.prototype.onStart = function () {
    ...
    // Run once shortly after plugin start, useful when USB is already inserted at boot.
    setTimeout(function () {
...
ControllerUsbAutoplay.prototype.onStop = function () {
    ...
    self.logger.info('[USB Autoplay] Stopping plugin');
...
ControllerUsbAutoplay.prototype.onRestart = function () {
    var self = this;
    self.logger.info('[USB Autoplay] Restarting plugin');
};
    ...
    self.hasUsbContent(usbPath)
        .then(function (available) {
            if (available && (!self.lastUsbAvailable || forceCheck)) {
                ...
                if (self.lastUsbAvailable) {
                    self.logger.info('[USB Autoplay] USB content no longer available');
                }
                self.lastUsbAvailable = false;
            }
        })
...
ControllerUsbAutoplay.prototype.runAutoplay = function () {
    ...
    self.waitForMpd()
        .then(function () {
            return self.execCommand('mpc update');
        })
        .then(function () {
            return self.waitForUsbContent(usbPath);
        })
        .then(function () {
            if (forcePlayback) {
        ...
        .then(function () {
            return self.execCommand('mpc clear');
        })
        .then(function () {
            return self.execCommand('mpc add "' + self.escapeShell(usbPath) + '"');
        })
        .then(function () {
            return self.execCommand('mpc random ' + random);
        })
        .then(function () {
            return self.execCommand('mpc repeat ' + repeat);
        })
        .then(function () {
            return self.execCommand('mpc single off');
        })
        .then(function () {
            return self.execCommand('mpc consume off');
        })
        .then(function () {
            return self.execCommand('mpc play');
        })
        .then(function () {
...
ControllerUsbAutoplay.prototype.hasUsbContent = function (usbPath) {
    ...
    self.execCommand('mpc listall "' + self.escapeShell(usbPath) + '"')
...
ControllerUsbAutoplay.prototype.escapeShell = function (value) {
