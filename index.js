...
ControllerUsbAutoplay.prototype.onStart = function () {
    ...
    this.logger.info("[USB Autoplay] Plugin started. udev/systemd trigger is used for USB insertion.");
...
ControllerUsbAutoplay.prototype.saveSettings = function (data) {
    ...
    try {
        ...
        this.config.set("usbPath", this.stringFromUi(data.usbPath, "USB"));
...
ControllerUsbAutoplay.prototype.writeRuntimeConfig = function () {
    this.writeRuntimeConfigWithEnabled(this.config.get("enabled"));
};

ControllerUsbAutoplay.prototype.writeRuntimeConfigWithEnabled = function (enabled) {
    var content = "";
    content += "ENABLED=" + (enabled ? "1" : "0") + "\n";
    content += "USB_PATH=" + this.shellString(this.config.get("usbPath") || "USB") + "\n";
    ...
    content += "MAX_WAIT_MPD=60\n";
    content += "MAX_WAIT_USB=120\n";

    try {
        fs.writeFileSync(this.runtimeConfigPath, content, "utf8");
        this.logger.info("[USB Autoplay] Runtime config written to " + this.runtimeConfigPath);
    } catch (e) {
        this.logger.error("[USB Autoplay] Could not write runtime config: " + e.message);
    }
};

ControllerUsbAutoplay.prototype.shellString = function (value) {
